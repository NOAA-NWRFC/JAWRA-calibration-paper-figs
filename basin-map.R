...
nwrfc_domain <- st_read("nwrfc-calibration-paper-data/NWRFC_Boundary/NWRFC_boundary.shp")
nwrfc_camels <- st_read("nwrfc-calibration-paper-data/CAMELS_NWRFC/CAMELS_NWRFC.shp")
# basins <- st_read("data/hlr-clustering/shape/basins_operational.shp")
basins <- st_read("nwrfc-calibration-paper-data/nwrfc_basins_sf/NWRFC_Forecast_Basins_20240113.shp")

states <- st_read("nwrfc-calibration-paper-data/cb_2018_us_state_5m/cb_2018_us_state_5m.shp") |>
  filter(NAME %in% c(
...
map
ggsave("figures/basin-map.png", width = 8, height = 8, dpi = 600)
